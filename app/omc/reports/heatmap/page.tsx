...
function heatColor(value: number, max: number) {
  ...
  const g = Math.round(244 - intensity * (244 - 130));
  const b = Math.round(236 - intensity * (236 - 60));
  return `rgb(${r},${g},${b})`;
}
...
export default async function HeatmapReportPage({ searchParams }: { searchParams: { degree?: string; batchId?: string } }) {
  ...
  if (!(await canViewReport(user, "omc.reports.heatmap"))) redirect("/dashboard");
  ...
  return (
    <Shell roleLabel={roleLabel(user.role)} userName={user.name} navLinks={navForRole(user.role)}>
      ...
      {programs.map((prog) => {
        const allVals = prog.plos.flatMap((p) => prog.courseTypes.map((t) => prog.matrix[p.number]?.[t] || 0));
            ...
            {prog.courseTypes.length === 0 || prog.plos.length === 0 ? (
              ...
              <SortableTable style={{ marginTop: 10 }}>
                <thead>
                  <tr>
                    <th>PLO</th>
                    {prog.courseTypes.map((t) => <th key={t} style={{ textAlign: "center" }}>{t}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {prog.plos.map((p) => (
                    <tr key={p.number}>
                      ...
                      {prog.courseTypes.map((t) => {
                        const val = prog.matrix[p.number]?.[t] || 0;
                        return (
                          <td key={t} style={{ textAlign: "center", background: heatColor(val, max), fontWeight: val > 0 ? 600 : 400, color: val / max > 0.6 ? "#fff" : "var(--ink)" }}>
                            {val}
                          </td>
                        );
                      })}
