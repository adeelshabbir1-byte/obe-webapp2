...
export default async function WeeklyPlanPage({ searchParams }: { searchParams: { courseId?: string } }) {
  ...
  const courses = await prisma.course.findMany({ where: { coordinatorId: { in: coordinatorIds } }, include: { batch: true, subjectExpert: true }, orderBy: { code: "asc" } });
