...
export async function POST(req: NextRequest) {
  ...
  for (const batch of pageOfBatches) {
    ...
    // Same real-FK-chain derivation as auto-map-hec-all/route.ts, but for
    // ploMappingSource = "SYSTEM" (unverified, keyword-inferred) instead of "HEC".
  ...
  await writeAuditLog({ actorUserId: user.id, action: "PLO_MAPPING_AUTO_MAPPED_SYSTEM_ALL_BATCHES", entityType: "Batch", entityId: "bulk", metadata: { offset, created: totalCreated } });
