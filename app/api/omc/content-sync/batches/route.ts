...
export async function GET() {
  try {
    ...
    const batches = await prisma.batch.findMany({
      ...
      select: { id: true, degreeProgram: true, batchName: true, startYear: true, _count: { select: { courses: true } } },
      orderBy: [{ degreeProgram: "asc" }, { batchName: "desc" }],
    });

    return NextResponse.json({
      batches: batches.map((b) => ({ id: b.id, degreeProgram: b.degreeProgram, batchName: b.batchName, startYear: b.startYear, courseCount: b._count.courses })),
    });
  } catch (err: any) {
