...
export async function PUT(req: NextRequest, { params }: { params: { courseId: string; lectureId: string } }) {
  ...
  if (!instrumentId || typeof linked !== "boolean") return NextResponse.json({ error: "instrumentId and linked are required" }, { status: 400 });
  ...
  if (linked) {
    await prisma.lectureRowInstrument.upsert({
      ...
      create: { lectureRowId: row.id, instrumentId }, update: {},
    });
  } else {
