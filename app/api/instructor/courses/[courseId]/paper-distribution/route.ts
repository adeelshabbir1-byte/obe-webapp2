...
export async function GET(req: NextRequest, { params }: { params: { courseId: string } }) {
  ...
  const items = await prisma.paperDistributionItem.findMany({ where: { courseId: course.id, source: "INSTRUCTOR" }, orderBy: { orderIndex: "asc" } });
...
export async function POST(req: NextRequest, { params }: { params: { courseId: string } }) {
  ...
  const count = await prisma.paperDistributionItem.count({ where: { courseId: course.id, source: "INSTRUCTOR" } });

  const item = await prisma.paperDistributionItem.create({
    data: {
      courseId: course.id, source: "INSTRUCTOR", questionNo: count + 1, orderIndex: count,
