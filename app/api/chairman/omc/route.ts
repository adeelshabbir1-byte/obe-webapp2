...
export async function GET() {
  ...
  return NextResponse.json({ omcMembers: omcMembers.map(({ passwordHash, ...u }) => u) });
}
...
export async function POST(req: NextRequest) {
  ...
  const created = await prisma.user.create({
    data: {
      ...
      role: "OMC", managedById: user.id, mustChangePassword: true,
    },
  });

  await writeAuditLog({ actorUserId: user.id, action: "OMC_MEMBER_CREATED", entityType: "User", entityId: created.id });
