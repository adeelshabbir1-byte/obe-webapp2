...
export default function InstrumentsManager({ courseId, initialInstruments, targets }: { courseId: string; initialInstruments: Instrument[]; targets: Targets }) {
  ...
  async function addInstrument(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    ...
    try {
      const res = await fetch(`/api/subjectexpert/courses/${courseId}/instruments`, {
        ...
        body: JSON.stringify({ type: fd.get("type"), label: fd.get("label"), marksPct: fd.get("marksPct") }),
      });
      ...
      {TYPES.map((type) => {
        ...
        return (
          <div className="card" key={type}>
            ...
            <table>
              <thead><tr><th>Label</th><th>Marks %</th><th></th></tr></thead>
              <tbody>
                {items.length === 0 && <tr><td colSpan={3} style={{ color: "var(--slate)" }}>None defined yet.</td></tr>}
                {items.map((i) => (
                  <tr key={i.id}>
                    <td>{i.label}</td><td>{i.marksPct}%</td>
            ...
            <form onSubmit={addInstrument} style={{ display: "flex", gap: 8, alignItems: "flex-end", marginTop: 10 }}>
              <input type="hidden" name="type" value={type} />
              <div><label style={{ fontSize: 11, color: "var(--slate)", display: "block", marginBottom: 4 }}>Label</label><input name="label" placeholder={type === "Midterm" || type === "Final" ? "Q1" : `${type} 1`} required style={{ padding: "6px 8px", border: "1px solid var(--line)", width: 100 }} /></div>
              <div><label style={{ fontSize: 11, color: "var(--slate)", display: "block", marginBottom: 4 }}>Marks %</label><input name="marksPct" type="number" min={0} max={100} required style={{ padding: "6px 8px", border: "1px solid var(--line)", width: 70 }} /></div>
