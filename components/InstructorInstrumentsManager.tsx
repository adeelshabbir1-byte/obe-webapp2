...
export default function InstructorInstrumentsManager({ courseId, initialInstruments, targets }: { courseId: string; initialInstruments: Instrument[]; targets: Targets }) {
  ...
  async function addInstrument(type: string, nextLabel: string, marksPct: string) {
    ...
    try {
      const res = await fetch(`/api/instructor/courses/${courseId}/instruments`, {
  ...
  async function removeInstrument(id: string) {
    setLoading(true);
    await fetch(`/api/instructor/courses/${courseId}/instruments/${id}`, { method: "DELETE" });
    setLoading(false); router.refresh();
  }
