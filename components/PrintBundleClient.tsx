...
export default function PrintBundleClient({ bundleName, reports }: { bundleName: string; reports: ReportDef[] }) {
  ...
  if (reports.length === 0) {
    return (
      <div style={{ padding: 40, fontFamily: "system-ui" }}>
        <p style={{ color: "#5B6B7C" }}>No reports in this bundle are available to you right now.</p>
      </div>
    );
  }
    ...
    return (
      <div>
        <style>{`
          ...
          .report-frame { width: 100%; height: 1100px; border: 1px solid #E4DFCE; margin-bottom: 20px; }
        `}</style>
        <div className="no-print" style={{ padding: "14px 20px", background: "#1E1B4B", color: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button onClick={() => window.print()} style={{ padding: "6px 14px", background: "#5B4FE8", color: "#fff", border: "none", cursor: "pointer" }}>Print All</button>
          </div>
        </div>
  ...
  return (
    <div style={{ padding: 40, fontFamily: "system-ui", maxWidth: 700, margin: "0 auto" }}>
      ...
      <p style={{ color: "#5B6B7C", fontSize: 13, marginBottom: 20 }}>{reports.length} report(s) in this bundle.</p>

      <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
        <button onClick={() => setMode("combined")} style={{ padding: "8px 16px", background: "#5B4FE8", color: "#fff", border: "none", cursor: "pointer" }}>
          View as One Combined Document
        </button>
      </div>

      <h3 style={{ fontSize: 14, marginBottom: 10 }}>Or Open Individually</h3>
      <p style={{ fontSize: 12, color: "#5B6B7C", marginBottom: 12 }}>Click each to open in a new tab and print it on its own — the most reliable way to get clean output per report.</p>
      <ul style={{ listStyle: "none", padding: 0 }}>
        {reports.map((r) => (
          <li key={r.id} style={{ marginBottom: 8 }}>
            <a href={r.href} target="_blank" rel="noopener noreferrer" style={{ color: "#4338CA", textDecoration: "underline", fontSize: 13.5 }}>{r.title}</a>
          </li>
        ))}
      </ul>
    </div>
