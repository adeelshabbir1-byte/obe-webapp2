...
export default function PrintBundleClient({ bundleName, reports }: { bundleName: string; reports: ReportDef[] }) {
  ...
  if (reports.length === 0) {
    return (
      <div style={{ padding: 40, fontFamily: "system-ui" }}>
        <p style={{ color: "#574C50" }}>No reports in this bundle are available to you right now.</p>
      </div>
    );
  }
  ...
  if (mode === "combined") {
    return (
      <div>
        ...
        `}</style>
        <div className="no-print" style={{ padding: "14px 20px", background: "#241A1D", color: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button onClick={() => window.print()} style={{ padding: "6px 14px", background: "#7E2435", color: "#fff", border: "none", cursor: "pointer" }}>Print All</button>
          </div>
        </div>
  ...
  return (
    <div style={{ padding: 40, fontFamily: "system-ui", maxWidth: 700, margin: "0 auto" }}>
      ...
      <p style={{ color: "#574C50", fontSize: 13, marginBottom: 20 }}>{reports.length} report(s) in this bundle.</p>

      <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
        <button onClick={() => setMode("combined")} style={{ padding: "8px 16px", background: "#7E2435", color: "#fff", border: "none", cursor: "pointer" }}>
          View as One Combined Document
        </button>
      </div>

      <h3 style={{ fontSize: 14, marginBottom: 10 }}>Or Open Individually</h3>
      <p style={{ fontSize: 12, color: "#574C50", marginBottom: 12 }}>Click each to open in a new tab and print it on its own — the most reliable way to get clean output per report.</p>
