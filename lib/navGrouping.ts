...
const RULES: { match: (href: string) => boolean; section: string }[] = [
  // Coordinator
  { match: (h) => /\/coordinator\/(faculty|batches|courses|plos|calendar|students)$/.test(h), section: "Setup" },
  { match: (h) => /\/coordinator\/(semester|repeat-offering)$/.test(h), section: "Semester Operations" },
  { match: (h) => /\/coordinator\/(load-report|semester-health|batch-comparison|prerequisite-map|feedforward-digest)$/.test(h), section: "Reports & Analytics" },
